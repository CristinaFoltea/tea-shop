app.controller('MainCtrl', ['$scope', function($scope){
  ...
  $scope.update_card = function(id, quantity){
    ...
    $scope.addItem[0].quantity = quantity
    $scope.shopingcard = $scope.shopingcard.concat($scope.addItem)
  }

  $scope.total = function(){
    return ($scope.shopingcard.reduce(function(result, item){
      return result += item.quantity * item.price
    }, 0))
  }

  $scope.remove = function(id){
    $scope.shopingcard = $scope.shopingcard.filter(function(item){
      return item._id !== id
    })
  }
}])
